const SchoolEngine = (() => {
  ...
  function rank(list, key) {
    const ordered = [...list].sort(
      (a, b) => b[key] - a[key] || a.name.localeCompare(b.name, 'es') || a.id - b.id
    );
    let rank = 0,
      previous = null;
  ...
  function calculate(data, state, weights, reference = [20, 20, 20, 20, 20]) {
    const w = normalize(weights),
      rw = normalize(reference);
    if (!w || !rw) return { rows: [], error: 'Asigna un peso mayor que cero a una materia.' };
    const years = [...state.years].sort(),
      yearSet = new Set(years),
      townSet = new Set(state.places ?? state.towns);
    if (!years.length || !townSet.size)
      return { rows: [], error: 'Selecciona al menos un municipio y un año.' };
    const schools = new Map(data.schools.map((s) => [s.id, s]));
    const groups = new Map();
    let sourceCount = 0;
    for (const r of data.records) {
      if (
        !townSet.has(state.places ? schools.get(r.id).place : schools.get(r.id).town) ||
        !yearSet.has(r.year)
      )
        continue;
      ...
      g.n += r.n;
      g.sourceRows.push(r.sourceRow);
      g.sessions.add(r.session);
      g.natures.add(r.nature);
      r.scores.forEach((v, i) => (g.sums[i] += v * r.n));
      sourceCount++;
    }
    ...
    for (const [id, byYear] of annual) {
      ...
      const scores = [0, 1, 2, 3, 4].map(
        (j) =>
          entries.reduce(
            (s, r) => s + r.scores[j] * (state.aggregation === 'students' ? r.n : 1),
            0
          ) / denominator
      );
      const change =
        years.length > 1 && byYear[years[0]] && byYear[years.at(-1)]
          ? byYear[years.at(-1)].score - byYear[years[0]].score
          : null;
      rows.push({
        ...schools.get(id),
        byYear,
    ...
    return {
      rows: rank(rows, 'score').map((r) => ({
        ...r,
        referenceRank: ref.get(r.id),
        rankChange: ref.get(r.id) - r.rank,
      })),
      years,
      weights: w,
      sourceCount,
